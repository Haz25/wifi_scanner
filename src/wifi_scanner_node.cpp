...
static std::vector<AP> run_scan() {
  ...
  const char* cmd = "/usr/bin/env LC_ALL=C /usr/bin/nmcli --colors no -t -f SSID,SIGNAL,BSSID dev wifi 2>/dev/null";
  std::string out;
  ...
  while (std::getline(iss, line)) {
    ...
    ap.bssid = line.substr(sep2+1);
    ap.power = std::stoi(line.substr(sep1+1, sep2-sep1-1)) / 5 * 5;
    v.push_back(ap);
  }
...
private:
  void tick() {
    ...
    sort(aps.begin(), aps.end(), [](AP &ap1, AP &ap2){
        return ap1.power != ap2.power? ap1.power > ap2.power: ap1.ssid < ap2.ssid;
    });
